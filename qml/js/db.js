...
function openDB() {
	if (db === null) {
		...
		db.transaction(function(tx){
			...
			tx.executeSql('CREATE TABLE IF NOT EXISTS poi_historial( id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, label TEXT UNIQUE, tag_online TEXT, tag_offline TEXT, enabled_offline TEXT )');
		});
	}
}
...
function saveToNearByHistory(label, tag_online, tag_offline, enabled_offline) {
	openDB();
	db.transaction( function(tx){
		tx.executeSql('INSERT OR REPLACE INTO poi_historial(label, tag_online, tag_offline, enabled_offline) VALUES(?,?,?,?)', [label, tag_online, tag_offline, enabled_offline]);
		tx.executeSql('DELETE FROM poi_historial WHERE id IN (SELECT id FROM poi_historial ORDER BY id DESC LIMIT -1 OFFSET 5)'); // Keep only 5 last
	});
}
...
function getNearByHistory() {
	...
	openDB();
	db.transaction(function(tx) {
		res = tx.executeSql('SELECT * FROM poi_historial ORDER BY id DESC', []);
	});
	return res;
}
